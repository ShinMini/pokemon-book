...
public class PokemonBookDAO {
		...
		try {
			...
			while (rset.next()) {
				list.add(PokemonBookDTO.pokemonBookDTOBuilder()
						...
						.pokemon(PokemonDAO.getPokemon(rset.getInt(2)))
						.owner(OwnerDAO.getOwner(rset.getInt(3))).build());
			}
		} finally {
	...
	/** 수정 UPDATE */
	// 포켓몬북 ID로 소유자 ID 수정
	public static boolean updatePokemonBookOwner(int pokemonBookId, OwnerDTO owner) throws SQLException {
		Connection con = null;
		PreparedStatement pstmt = null;
		try {
			con = DBUtil.getConnection();
	...
	// 포켓몬북 ID로 포켓몬 ID 수정
	public static boolean updatePokemonBookPokemon(int pokemonBookId, PokemonDTO pokemon) throws SQLException {
		Connection con = null;
		PreparedStatement pstmt = null;
		try {
			con = DBUtil.getConnection();
